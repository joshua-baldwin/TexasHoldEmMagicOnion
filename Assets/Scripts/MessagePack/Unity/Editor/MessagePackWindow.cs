...
namespace MessagePack.Unity.Editor
{
    internal class MessagePackWindow : EditorWindow
    {
        ...
        async void OnGUI()
        {
            ...
            if (GUILayout.Button("Generate"))
            {
                ...
                try
                {
                    var log = await ProcessHelper.InvokeProcessStartAsync("/Users/joshua_baldwin/.dotnet/tools/mpc", "mpc " + commnadLineArguments);
                    UnityEngine.Debug.Log(log);
                }
                finally
                {
    ...
    internal static class ProcessHelper
    {
        ...
        {
            return await InvokeProcessStartAsync("/usr/local/share/dotnet/dotnet", "tool install --global " + InstallName);
        }
        ...
        public static async Task<(bool found, string version)> FindDotnetAsync()
        {
            try
            {
                var version = await InvokeProcessStartAsync("/usr/local/share/dotnet/dotnet", "--version");
                return (true, version);
            }
            catch
            {
